...
int sys_set_env_status(u_int envid, u_int status) {
	...
	if(env->env_status == ENV_NOT_RUNNABLE && status == ENV_RUNNABLE) TAILQ_INSERT_HEAD(&env_sched_list, env, env_sched_link);
	if(env->env_status == ENV_RUNNABLE && status == ENV_NOT_RUNNABLE) TAILQ_REMOVE(&env_sched_list, env, env_sched_link);
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		int res = sys_mem_map(curenv->env_id, srcva, envid, e->env_ipc_dstva, perm);
		if(res!=0) return res;
	}
	return 0;
}
