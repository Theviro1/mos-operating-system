...
void page_init(void) {
	...
	extern char end[];
	int alloced_page_num = (freemem - (u_long)end) / BY2PG;
	int i;
	for(i=0;i<alloced_page_num;i++){
		pages[i].pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for(i=alloced_page_num;i<npage;i++){
		pages[i].pp_ref = 0;
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	Pte *pgtbl_entryp = (Pte*)(KADDR(PTE_ADDR(*pgdir_entryp))) + PTX(va);
	*ppte = pgtbl_entryp;
	return 0;
}
