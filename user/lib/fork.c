...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if((perm & PTE_COW) == 0) user_panic("invalid raise, PTE_COW of %d is 0", va);
	...
	int r = syscall_mem_alloc(0, UCOW, perm);
	if(r) user_panic("fail to syscall_mem_alloc in function cow_entry\n");
	...
	r = syscall_mem_map(0, UCOW, 0, va, perm);
	if(r) user_panic("fail to syscall_mem_map with UCOW\n");
	// Step 6: Unmap the page at 'UCOW'.
	/* Exercise 4.13: Your code here. (6/6) */
	r = syscall_mem_unmap(0, UCOW);
	if(r) user_panic("fail to syscall_mem_unmap in function cow_entry\n");
...
static void duppage(u_int envid, u_int vpn) {
	...
	perm = (*entry) & 0xfff;
	//先判断entry这个页面是否是只读的，如果只读直接传递给子进程
	if((perm & PTE_D) == 0){
		r = syscall_mem_map(0, addr, envid, addr, perm);
	       if(r) return r;
	}
	//不是只读，再判断这个页面是否是共享的，如果是共享也直接传递给子进程
	else if((perm & PTE_LIBRARY) != 0){//不是0说明perm的PTE_LIBRARY位不是0 说明是共享页面
		r = syscall_mem_map(0, addr, envid, addr, perm);
		if(r) user_panic("fail to syscall_mem_map with PTE_LIBRARY\n");
	}
	//不是只读，不是共享，再判断这个页面是否是写时复制的，如果是写时复制的也直接传递给子进程
	else if((perm & PTE_COW) != 0){
		r = syscall_mem_map(0, addr, envid, addr, perm);
		if(r) user_panic("fail to syscall_mem_map with PTE_COW\n");
	}
	//都不是上述情况就是可写入的，这时候需要把父子进程里的权限PTE_COW都置为1 PTE_D都置为0
	else{
		perm = perm & (~PTE_D);
		perm = perm | PTE_COW;
		r = syscall_mem_map(0, addr, envid, addr, perm);//给子进程赋予正确的权限位
		if(r) user_panic("fail to syscall_mem_map with PTE_D to child\n");
		r = syscall_mem_map(0, addr, 0, addr, perm);//给父进程自己赋予正确的权限位
		if(r) user_panic("fail to syscall_mem_map with PTE_D to parent\n");
	}
}
...
int fork(void) {
	...
	for(i=0;i<VPN(USTACKTOP);i++){
		if( ( (vpt[i] & PTE_V)!=0 ) && ( (vpd[(i>>10)] & PTE_V)!=0 ) ){
			duppage(child, i);
		}
	}
